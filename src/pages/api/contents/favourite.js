...
export default async function getMyFavourite(req, res) {
    const {token, startCursor, pageSize} = req.body
    const payload = vertifyJwtToken(token, JWT_SECURE_KEY)
    
    if (payload) {        
        const data = await contentsRepo.getMyFavourite(payload?.notion_id, startCursor, pageSize)
        console.log("getMyFavourite: ", data)
        ...
        console.log(data?.results?.[0]?.properties?.collection_name?.rollup?.array?.[0]?.title?.[0]?.plain_text)
        console.log(data?.results?.[0]?.properties?.collections)
